...
export default function App({ onSignOut, currentUsername, currentUserId }) {
  ...
  return (
    <div style={{ fontFamily: "'Pretendard','Apple SD Gothic Neo',system-ui,sans-serif", background: C.bg, minHeight: "100%", color: C.ink }}>
      <TopBar tab={tab} setTab={setTab} date={date} setDate={setDate} onSignOut={onSignOut} currentUsername={currentUsername} />
      <div style={{ maxWidth: 1120, margin: "0 auto", padding: "20px 20px 60px" }}>
        ...
        {tab === "calendar" && <CalendarView data={data} updateData={updateData} />}
        {tab === "students" && <StudentView data={data} updateData={updateData} />}
        {tab === "teachers" && <TeacherView data={data} updateData={updateData} currentTeacherId={currentTeacherId} />}
